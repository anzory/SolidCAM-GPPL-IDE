...
- Restore compatibility with the SolidCAM GPP Debugger extension (`solidcam.gpp-debug`) when both extensions are installed: breakpoints can now be set in GPPL files, and F5 supplies the default "Attach" debug configuration.
- Serve IDE language features (diagnostics, hover, completion, formatting) for `.gpp` files opened in the `gpp` language mode used by SolidCAM's own extensions, so the debugger and the IDE work side by side.
